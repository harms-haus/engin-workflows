...
export interface DebugWorkflowOptions {
...
export async function finalReviewPhase(
    ...
    for (let round = 0; round < maxFixRounds; round++) {
        ...
        const pool = new LanePool({
            maxConcurrentLanes: maxConcurrentTasks ?? 3,
            profilesDirs,
...
export async function run(
    ...
    const profilesDirs: string[] = options.profilesDir ? [options.profilesDir] : resolveProfilesDirs(options.cwd, 'debug');
