...
2. **READABILITY**: Misleading or unclear names, raw magic numbers/strings that should be named constants, overly long or deeply nested functions (god functions), confusing control flow, **shorthand hiding core behavior** (important logic written as dense/overly-clever one-liners where an explicit form reads better — terseness belongs on small, low-stakes steps), **verbose/redundant comments** that restate obvious code (prefer self-documenting code; comment only the non-obvious), comments that contradict the code, **tracker refs** (`task #` / `issue #` / ticket IDs), and **timely/past-rationale comments** describing *why it was done* or past/"old way" behavior ("previously…", "used to…", "legacy") instead of *why it IS* (current behavior). Also flag missing docstrings on public/exported functions and types (public-API contract docs are valuable — do NOT treat them as verbose noise).

3. **MAINTAINABILITY**: Duplicated logic (DRY violations) that should be extracted into a shared utility, tight coupling, missing or leaky abstractions, code that resists change, functions/files doing more than one thing (**god functions** and **monolith files** that should be decomposed/split), **name/file mismatches** where a type or function's name doesn't match the file it lives in or is exported from, and **useless barrels / mis-routed imports** — modules that only re-export another module with no added value, and imports routed through such a barrel instead of the real source. Officialize the real source: import directly, or, where a wrapper genuinely adds value, make it a proper first-class module. A legitimate facade providing a single stable entry point is NOT a useless barrel.
