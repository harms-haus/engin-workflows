...
7. **MISSING DOCSTRINGS ON PUBLIC API**: Exported/public functions, classes, and types added or substantially changed by the changeset that lack docstrings/TSDoc/JSDoc documenting purpose, parameters, return values, thrown errors, or notable behavior.
8. **DOCSTRINGS/DOCS THAT DON'T MATCH THE CODE**: Doc comments or external docs whose signatures, types, defaults, thrown errors, or described behavior no longer match the real code — including **tracker refs** (`task #` / `issue #` / ticket IDs) and **timely/past-rationale comments** inside doc text that reference past/"old way" behavior or explain *why it was done* rather than *why it IS*. Validate every docstring against the actual implementation.
