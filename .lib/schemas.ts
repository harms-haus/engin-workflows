...
export const FinalReviewSeveritySchema = z.enum(["low", "medium", "high", "critical"]);
export type FinalReviewSeverity = z.infer<typeof FinalReviewSeveritySchema>;

export const FinalReviewFindingSchema = z.object({
    id: z
        .string()
        .describe("Short stable kebab-case identifier, unique within this review (e.g. 'n-plus-one-user-query')"),
    ...
    file: z.string().describe("Primary file the finding relates to, with line range if helpful (e.g. 'src/auth.ts:42-58')"),
    title: z.string().describe("Concise one-line summary of the finding"),
    description: z.string().describe("Detailed explanation: what is wrong, why it matters, and the intended correct behavior"),
    fixPrompt: z
        .string()
        .describe(
...
export const FinalReviewResultSchema = z.object({
    dimension: z.string().describe("The review dimension you assessed (must match the dimension assigned in the prompt)"),
    applicable: z
        .boolean()
        .describe(
    ...
    notApplicableReason: z
        .string()
        .describe("Required when applicable=false. Explain why this dimension is irrelevant to the current changes. Empty string when applicable=true."),
    summary: z.string().describe("Overall assessment for this dimension (what you checked and your conclusion)"),
    findings: z
        .array(FinalReviewFindingSchema)
        .describe("Findings ordered by severity, most severe first. Empty when applicable=false or when there are genuinely no issues."),
});
